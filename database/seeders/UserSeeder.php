<?php

namespace Database\Seeders;

use App\Models\User;
use Illuminate\Database\Seeder;

class UserSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        $user = new User();
        $user->password = "12345";
        $user->identification = 200178;
        $user->name = "Carlos Alpizar Gutierrez";
        $user->birthdate = '1999-04-21';
        $user->phone = 83451018;
        $user->email = "carlosalpizarg@hotmail.com";
        $user->address = "Alajuela, Alajuela, El Roble";
        $user->gendet = 'M';
        $user->role_id = 1;
        $user->save();

        $user1 = new User();
        $user1->password = "12345";
        $user1->identification = 0342;
        $user1->name = "Josue Perez Sequeira";
        $user1->birthdate = '1997-02-04';
        $user1->phone = 86155929;
        $user1->email = "jdps04@hotmail.es";
        $user1->address = "Alajuela,Grecia, Grecia";
        $user1->gendet = 'M';
        $user1->role_id = 1;
        $user1->save();

        $user2 = new User();
        $user2->password = "12345";
        $user2->identification = 20556;
        $user2->name = "atleta1";
        $user2->birthdate = '1999-04-21';
        $user2->phone = 4561616;
        $user2->email = "billy@hotmail.com";
        $user2->address = "Limon, Siquirres, Turri";
        $user2->gendet = 'M';
        $user2->role_id = 3;
        $user2->save();

        $user3 = new User();
        $user3->password = "4555";
        $user3->identification = 25;
        $user3->name = "Atleta 2";
        $user3->birthdate = '1999-04-21';
        $user3->phone = 834510148;
        $user3->email = "44@hotmail.com";
        $user3->address = "Alajuela, Alajuela, El Roble";
        $user3->gendet = 'M';
        $user3->role_id = 3;
        $user3->save();

        $user4 = new User();
        $user4->password = "12545";
        $user4->identification = 55;
        $user4->name = "Atheta3";
        $user4->birthdate = '1999-04-21';
        $user4->phone = 834512018;
        $user4->email = "vn@hotmail.com";
        $user4->address = "Alajuela, Alajuela, El Roble";
        $user4->gendet = 'M';
        $user4->role_id = 3;
        $user4->save();

        $user5 = new User();
        $user5->password = "125hm45";
        $user5->identification = 45;
        $user5->name = "entrenador 1";
        $user5->birthdate = '1999-04-21';
        $user5->phone = 834512018;
        $user5->email = "hhgvn@hotmail.com";
        $user5->address = "Alajuela, Alajuela, El Roble";
        $user5->gendet = 'M';
        $user5->role_id = 2;
        $user5->save();

        $user6 = new User();
        $user6->password = "12545";
        $user6->identification = 1;
        $user6->name = "entrenador 2";
        $user6->birthdate = '1999-04-21';
        $user6->phone = 83562018;
        $user6->email = "n@11hotmail.com";
        $user6->address = "Alajuela, Alajuela, El Roble";
        $user6->gendet = 'M';
        $user6->role_id = 2;
        $user6->save();


        $user7 = new User();
        $user7->password = "125hbm45";
        $user7->identification = 575;
        $user7->name = "entrenador 3";
        $user7->birthdate = '1999-04-21';
        $user7->phone = 835620138;
        $user7->email = "@11hotmail.com";
        $user7->address = "Alajuela, Alajuela, El Roble";
        $user7->gendet = 'M';
        $user7->role_id = 2;
        $user7->save();
    }
}
